...
Script to take separate PBA_TRSE_Task_2 outputs and compile into a single
    matrix. Task 2 outputs are LSOA -> NoRMS -> NoRMS -> LSOA journeys

NB: THIS SCRIPT IS ONLY REQUIRED IF YOU HAVE MULTIPLE OTP RESPONSE FILES, THUS
     MULTIPLE OUTPUTS FROM EACH OTP FILE. THIS SCRIPT CONCATENATES THE SEPARATE
     OUTPUT FILES INTO A SINGLE OUTPUT.
...
data_dir = r'Y:\PBA\Analysis\Data(copy)\OTP data'

for code in run_codes:
    run_code, run_year = code.split(' ')
    print('\nNow on:', run_code, run_year)

    low_amenity_run_fn = '45_mins_max_time__low_amenity_north_(fixed_tsk2)BUS_WALK_rail_jt_' + run_code + '_' + run_year + '_.csv'
    GM_test_run_fn = '45_mins_max_time__GM_test_run(fixed_tsk2)BUS_WALK_rail_jt_' + run_code + '_' + run_year + '_.csv'
    north_run_800k_fn = '45_mins_max_time__800k_interupted_north_run(fixed_tsk2)BUS_WALK_rail_jt_' + run_code + '_' + run_year + '_.csv'
    rest_of_north_fn = '45_mins_max_time__rest_of_north(pen_run)BUS_WALK_rail_jt_' + run_code + '_' + run_year + '_.csv'
    debug_run_800k_fn = '45_mins_max_time__final_debug_run(fixed_tsk2)BUS_WALK_rail_jt_' + run_code + '_' + run_year + '_.csv'
    ...
    #### 4 Format & export
    print("\nExporting data for", len(compiled_matrix['o'].unique()), "unique origins and",
          len(compiled_matrix['d'].unique()), "destinations")
    print(time.strftime("%H:%M:%S - %Y", time.localtime()))
    compiled_matrix = compiled_matrix[['o', 'd', 'jt', 'bus_pct', 'rail_pct']]
    print(time.strftime("%H:%M:%S - %Y", time.localtime()), '\nDone')

    save_name = run_code + '_' + run_year + '_compiled_walk_bus_rail_jt.csv'
